...
typedef struct instruction{
  int type; //2 - store; 1- load; 0 - outras
  int write;
  int read1;
  int read2;
} instruction;
...
/* Armazenam numero de instrucoes, ciclos e data stalls */
long instruct = 0;
long cycles = 0;
long data_stall = 0;
instruction primeira, segunda, nop_instruct;
instruction pipeline_ALU[5],pipeline_lw_sw[5];
queue<instruction> buffer;

#define DEFAULT_STACK_SIZE (256*1024)

void update_pipeline(instruction pipeline[5], instruction new_inst){
...
void reset_pipeline(instruction pipeline[5]){
...
void data_hazard_check(int type, int write, int read1, int read2){
    instruction new_inst;
    
    new_inst.type  = type;
    new_inst.write     = write;
    new_inst.read1     = read1;
    new_inst.read2     = read2;
    
    instruct++;
    
    buffer.push(new_inst);
    
    //Atualiza os pipelines
    if(buffer.size() >= 2){
    	primeira = buffer.front();
    	buffer.pop();
    	segunda = buffer.front();
    	buffer.pop();
    	cycles++;
    	

    	//Só uma instrução pode ser colocada no pipeline
    	if(primeira.type == segunda.type){
    		buffer.push(segunda);
    		if(primeira.type == 0){
    			update_pipeline(pipeline_ALU, primeira);
    			update_pipeline(pipeline_lw_sw, nop_instruct);
    		}
    		else{
    			update_pipeline(pipeline_lw_sw, primeira);
    			update_pipeline(pipeline_ALU, nop_instruct);
    		}
    	}
    	

    	//Duas instruções podem ser colocadas no pipeline
    	else{
    		if(primeira.type == 0){
    			update_pipeline(pipeline_ALU, primeira);
    			update_pipeline(pipeline_lw_sw, segunda);
    		}
    		else{
    			update_pipeline(pipeline_ALU, segunda);
    			update_pipeline(pipeline_lw_sw, primeira);
    		}
    	}
    ...
    //Verifica se o dados escrito vai ser usado no mesmo ciclo
    if((pipeline_ALU[2].write != -1) && (pipeline_ALU[2].write == pipeline_lw_sw[2].read1|| pipeline_ALU[2].write ==pipeline_lw_sw[2].read2)){
    	data_stall++;
    }
    else if(pipeline_lw_sw[2].type == 1){
        if(pipeline_lw_sw[2].write == pipeline_ALU[2].read1){
            data_stall+=2;
        }
        else if(pipeline_lw_sw[2].write == pipeline_ALU[2].read2){
            data_stall+=2;
        }
        else if(pipeline_lw_sw[2].write == pipeline_ALU[1].read1){
            data_stall++;
        }
        else if(pipeline_lw_sw[2].write == pipeline_ALU[1].read2){
            data_stall++;
        }
    }
...
void ac_behavior(begin)
{
  ...
  reset_pipeline(pipeline_ALU);
  reset_pipeline(pipeline_lw_sw);
  
  nop_instruct.type = -1;
  nop_instruct.write= -1;
  nop_instruct.read1= -1;
  nop_instruct.read2= -1;
}

//!Behavior called after finishing simulation
void ac_behavior(end)
{
  printf("Número de instruções executadas:		%d\n", instruct);
  printf("Número de ciclos:				%d\n", cycles);
  printf("Número de data hazards:				%d\n", data_stall);
  printf("Número de ciclos total:			%d\n", cycles+data_stall);
