...
typedef struct instruction{
  int mem_read; //Indica se houve leitura de dados da memoria
  int write; //Registrador onde é escrito dado
  int read1; //Registrador de leitura
  int read2; //Registrador de leitura
} instruction;
...
int cycles = 0;
int data_stall = 0;
instruction atual,anterior;

instruction pipeline[5];
#define DEFAULT_STACK_SIZE (256*1024)

void update_pipeline(instruction new_inst){
...
void data_hazard_check(int mem_read, int write, int read1, int read2){
    instruction new_inst;
    
    new_inst.mem_read  = mem_read;
    new_inst.write = write;
    new_inst.read1 = read1;
    new_inst.read2 = read2;
    
    update_pipeline(new_inst);
    
    //Verifica caso um hazard acontece
    if(pipeline[2].mem_read == 1){
    	contador_load++;
        if(pipeline[2].write == pipeline[1].read1)
            data_stall++;
        else if(pipeline[2].write == pipeline[1].read2)
            data_stall++;
    }
...
void ac_behavior(begin)
{
  ...
  pipeline[2].mem_read = 0;
  pipeline[1].mem_read = 0;
  pipeline[0].mem_read = 0;
}
